...
- [x] Security review:
  ...
  - **Secrets**: `appsettings.json` carries shape only — keys (`Stripe:SecretKey`, `Jwt:SigningKey`, `Notifications:ResendApiKey`, `Notifications:VapidPrivateKey`) come from `appsettings.{Environment}.Local.json` (gitignored) or environment variables. `.gitignore` blocks `*.Local.json` from being committed.
