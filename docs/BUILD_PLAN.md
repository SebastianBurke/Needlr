...
- [x] Implement `IArtistDiscoveryService` in Application layer with method `SearchAsync(SearchCriteria, CancellationToken)`
  - [x] Bounding-box spatial filter — uses literal X/Y axis comparisons on the Point column (planner picks them up cleanly with the GiST index; sidesteps antimeridian-wrap which a single-market launch doesn't hit). `ST_Within` is available if we need a true polygon test later — required when we add a market spanning the antimeridian.
...
- **Default deposit**: a single $100 CAD constant (`BookingDefaults.DefaultDepositCad`) is used at request time. Per-artist overrides + the actual Stripe Payment Intent amount come in Phase 11. Currency is hardcoded CAD for the Montréal launch — non-CAD markets will require parameterizing currency per jurisdiction (see `docs/FEATURE_SPECS.md` § Adding markets).
...
- [x] **Discovery filter refresh** (post-Phase 17 polish): drop the user-configurable Sort select (always order by earliest-bookable-day → distance tiebreaker); expose canonical-style multi-select chips backed by new `GET /api/styles/canonical`; add a venue-level **Walk-ins welcome** filter backed by new `Studio.AcceptsWalkIns` boolean (migration `AddStudioAcceptsWalkIns`). `DiscoverySort` enum collapsed to a single value; `DiscoverySearchCriteria` and `DiscoverySearchArgs` lose the `Sort` field, gain `AcceptsWalkInsOnly`. Dev fixture seeds ~30% walk-in studios. Map production polish (tile-provider swap, custom style, clustering, bounded camera) remains deferred to a follow-up branch.
...
Notes:
- **Style multi-select UI shipped in the post-Phase 17 polish.** Chip-style filter on Home.razor backed by `GET /api/styles/canonical`; selected ids feed `DiscoverySearchArgs.StyleIds`.
...
- **Map clustering at low zoom deferred.** MapLibre's clustering requires source data, not individual marker DOM nodes; v1 caps at ~50 results (`PageSize: 50`) which doesn't cluster meaningfully in a single launch-market viewport. Re-evaluate when seeded data per market exceeds that.
